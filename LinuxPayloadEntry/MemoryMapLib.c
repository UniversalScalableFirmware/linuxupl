...
{
  return (unsigned char)(Guid == 0 && ((CONST UINT64 *)(Guid + 1)) == 0);
}
...
GetSmallestKMemoryAllocationHob (
  EFI_HOB_MEMORY_ALLOCATION  **MemoryAllocationHobPtr,
  UINTN                      K,
  ...
  )
{
  ...
  unsigned char                    EnumAllHobs;

  EnumAllHobs =  ((unsigned char)(0==1));
  ...
  )
{
  ...
  while (!END_OF_HOB_LIST (Hob)) {
        ...
        ((Hob.ResourceDescriptor->ResourceType == EFI_RESOURCE_SYSTEM_MEMORY) || (Hob.ResourceDescriptor->ResourceType == EFI_RESOURCE_MEMORY_RESERVED)))
    {
      ...
      if ((SmallestResourceHob == ((void *) 0)) || (SmallestResourceHob->PhysicalStart > Hob.ResourceDescriptor->PhysicalStart)) {
  ...
  )
{
  if ((CurrentInfo->CurrentEnd == MemoryBaseAddress) &&
      ...
      (CurrentInfo->NewElement ==  ((unsigned char)(0==1))))
  {
  ...
  )
{
  ...
  for (Round = 0; Round < 2; Round++) {
    ...
    CurrentInfo.Count      = 0;
    CurrentInfo.NewElement =  ((unsigned char)(1==1));
    CurrentResourceHob     = GetSmallestResourceHob (CurrentInfo.CurrentEnd);
    while (CurrentResourceHob != ((void *) 0)) {
      //
  ...
  *Count = (UINT16)CurrentInfo.Count;
